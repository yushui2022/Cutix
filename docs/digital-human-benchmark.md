...
首页右侧“任务状态”卡片会显示最近一次压测的成功率、平均耗时、P95、输出体积和健康检查状态。

当页面已经生成 TTS 音频，并且系统设置里保存了本地数字人 HTTP endpoint 时，可以点击“用首段 TTS 启动压测”。Web 端会把当前第一段 TTS 的 `/output/...` 音频交给后台 benchmark 子进程，报告仍写入同一个 `benchmarks/` 目录。为了避免浏览器请求任意读取本机文件，Web 启动模式只接受 Cutix 自己的 `/output` 或 `/uploads` 音频；需要自定义绝对音频路径时继续使用 CLI。
